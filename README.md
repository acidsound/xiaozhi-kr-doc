...
```text
archive_rebuild/markdown/     중국어 원문 Markdown 79개
archive_rebuild/markdown_kr/  한국어 번역 Markdown 55개
archive_rebuild/assets/       Markdown에서 참조하는 이미지 파일 722개
```

한국어 번역본은 원문과 같은 파일명을 사용합니다. 본문 제목과 설명은 한국어로 옮겼고, URL, 펌웨어 파일명, 보드 타입, GPIO, 명령어 같은 기술 식별자는 그대로 유지했습니다.

`archive_rebuild/`는 현재 기준의 로컬 작업 산출물입니다. GitHub에 올릴 소스 저장소에는 포함하지 않고, 필요할 때만 이 산출물로부터 GitHub Wiki용 페이지를 별도로 생성하는 구조를 사용합니다.
...
Markdown 본문의 이미지 링크는 원격 Feishu URL을 그대로 두지 않고 `archive_rebuild/assets/` 아래 파일로 내려받은 뒤 상대 경로로 연결합니다.
...
```sh
python3 scripts/archive_feishu.py --recursive --depth -1 --max-pages 100 --no-raw --out archive_rebuild/markdown 'https://my.feishu.cn/wiki/F5krwD16viZoF0kKkvDcrZNYnhb'
```

## GitHub Wiki로 내보내기

소스 저장소에는 `archive_rebuild/`를 커밋하지 않고, 로컬 산출물을 읽어 GitHub Wiki용 별도 디렉터리를 생성합니다. 이 저장소에서는 `.wiki-build/`가 그 출력 위치입니다.
...
- `archive_rebuild/assets/` 이미지를 `images/`로 복사하고 링크 수정
...
```sh
python3 scripts/archive_feishu.py --fetch-url 'https://smvsudqc87.feishu.cn/docx/AjtQdM0KpoVQ3YxJfaLco4KsnSe' --out archive_rebuild/markdown 'https://my.feishu.cn/wiki/AjtQdM0KpoVQ3YxJfaLco4KsnSe'
```
...
```sh
python3 scripts/archive_feishu.py --html 'archive_rebuild/markdown/_________________________________________________【最新】小智AI面包板带摄像头互动功能接线教程 - Feishu Docs.html' --out archive_rebuild/markdown 'https://my.feishu.cn/wiki/GoGBdLVUooHARPxyTLhc9AjdnFh'
```
